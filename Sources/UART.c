...
void InitUARTs(){
	...
	UART0_C2 = UARTLP_C2_RE_MASK | UART_C2_RIE_MASK | UARTLP_C2_TE_MASK ;//| UART_C2_TIE_MASK; // Enable Transmitter, Receiver, Transmitter interrupt
...
void RF433MHz_transmitter(UART_MemMapPtr channel,char* Buffer){
	UARTprintf(UART0_BASE_PTR,"qwer"); // qwer is the Preemble password
	UARTprintf(UART0_BASE_PTR,Buffer); // send data frame buffer
	uart_putchar (UART0_BASE_PTR,0); // End data frame character
}
